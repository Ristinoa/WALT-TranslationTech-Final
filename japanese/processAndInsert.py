...
# ! This file is configured to work only on JAPANESE files
...
strings_block_start = re.compile(
    r'^\s*translate\s+japanese\s+strings\s*:\s*$',
    re.IGNORECASE
)
...
def clean_line_text(text):
    """
    Remove many styles of quotes and normalize text for insertion.
    - Strips ASCII and smart quotes (“ ” ‘ ’)
    - Strips Japanese corner quotes 「 」
    - Strips Chinese corner quotes 『 』 if present
    - Trims both normal and full-width spaces
    """
    if text is None:
        return ""
...
if __name__ == "__main__":
    ...
    processed_files = ["script_translated_JA.txt", "common_extracted_JA.txt", "screens_extracted_JA.txt"]
    outputs = ["script_translated_final_JA.rpy", "common_translated_final_JA.rpy", "screens_translated_final_JA.rpy"]
